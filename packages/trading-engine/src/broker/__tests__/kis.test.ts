...
describe('KisBrokerAdapter', () => {
  test('connect and disconnect lifecycle', async () => {
    const adapter = new KisBrokerAdapter({ apiKey: 'kis-key', secretKey: 'kis-secret', isPaper: true })
  ...
  test('returns balance and order response', async () => {
    ...
    const order = await adapter.placeOrder({ symbol: '005930', side: 'BUY', quantity: 1, type: 'MARKET' })
